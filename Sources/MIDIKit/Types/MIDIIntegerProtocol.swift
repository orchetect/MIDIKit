...
public protocol MIDIIntegerProtocol {
    // Storage
...
extension MIDIIntegerProtocol {
    ...
    public init<T: BinaryInteger>(clamping source: T) {
        let clamped = Storage(
            Int(source)
                .clamped(to: Self.min(Int.self) ... Self.max(Int.self))
        )
        self.init(clamped)
    }
    ...
    public init?<T: BinaryFloatingPoint>(exactly source: T) {
        ...
        // largest MIDIIntegerProtocol concrete type we're using (UInt14).
        // the smallest floating point number in the Swift standard library
        // is Float16 which can hold UInt14.max fine.
...
extension BinaryInteger {
    /// Creates a new instance from the given integer.
    public init<T: MIDIIntegerProtocol>(_ source: T) {
        self.init(source.value)
    }
    
    /// Creates a new instance from the given integer, if it can be represented exactly.
    public init?<T: MIDIIntegerProtocol>(exactly source: T) {
