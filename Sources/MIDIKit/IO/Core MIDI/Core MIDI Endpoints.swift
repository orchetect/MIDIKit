...
extension MIDI.IO {
    ...
    internal static func getSystemSourceEndpoints(matching name: String)
    -> [MIDIEndpointRef] {
    ...
    internal static func getSystemSourceEndpoint(matching uniqueID: MIDIUniqueID)
    -> MIDIEndpointRef? {
        for i in 0 ..< MIDIGetNumberOfSources() {
            let endpoint = MIDIGetSource(i)
...
extension MIDI.IO {
    ...
    ///
    /// - Parameter name: MIDI port name to search for.
    internal static func getSystemDestinationEndpoints(matching name: String)
    -> [MIDIEndpointRef] {
        var refs: [MIDIEndpointRef] = []
    ...
    ///
    /// - Parameter uniqueID: MIDI port unique ID to search for.
    internal static func getSystemDestinationEndpoint(matching uniqueID: MIDIUniqueID)
    -> MIDIEndpointRef? {
        for i in 0 ..< MIDIGetNumberOfDestinations() {
            let endpoint = MIDIGetDestination(i)
            if MIDI.IO.getUniqueID(of: endpoint) == uniqueID { return endpoint }
