//
//  UndefinedChunk+MIDIFileChunkIdentifier.swift
...
public struct UndefinedMIDIFileChunkIdentifier: MIDIFileChunkIdentifier {
    ...
    static var disallowedIdentifiers: [String] {
        [
            HeaderMIDIFileChunkIdentifier().string,
            TrackMIDIFileChunkIdentifier().string
        ]
    }
}
...
extension MIDIFileChunkIdentifier where Self == UndefinedMIDIFileChunkIdentifier {
    /// Undefined (non-standard) chunk identifier.
    /// This identifier must be different from the header identifier (`MThd`) and the track identifier (`MTrk`).
    public static func undefined(identifier: String) -> UndefinedMIDIFileChunkIdentifier? {
        UndefinedMIDIFileChunkIdentifier(string: identifier)
    }
}
