//
//  AnyMIDIFileHeaderChunk+Decoding.swift
...
extension AnyMIDIFileHeaderChunk {
    ...
    static func initFrom<D: DataProtocol>(
        ...
        return try stream.withDataParser { parser throws(MIDIFileDecodeError) in
            ...
            guard (try? parser.read(bytes: 4).toUInt8Bytes()) == HeaderMIDIFileChunkIdentifier().string.toASCIIBytes()
            else {
                throw .malformed(
    ...
    static func initFrom<D: DataProtocol>(
        ...
        return try midi1SMFRawBytes.withDataParser { parser throws(MIDIFileDecodeError) in
            ...
            guard let timebase = AnyMIDIFileTimebase(data: timeDivision) else {
                throw .malformed(
