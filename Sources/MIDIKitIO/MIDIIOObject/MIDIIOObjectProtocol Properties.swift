...
extension MIDIIOObject {
    ...
    /// (`kMIDIPropertyName`)
    ///
    /// Devices, entities, and endpoints may all have names. The standard way to display an
    /// endpoint’s name is to ask it for its name and display it only if unique. If not, prepend the
    /// device name.
    ///
    ...
    /// Get the device’s current patch, note, and control name values in MIDINameDocument XML
    /// format.
    ...
    /// (`kMIDIPropertyImage`)
    ///
    /// You can provide an image stored in any standard graphic file format, such as JPEG, GIF, or
    /// PNG. The maximum size for this image is 128 by 128 pixels.
    ///
    ...
    /// Get a Boolean value that indicates whether the MIDI pan messages sent to the device or
