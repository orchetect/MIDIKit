...
extension MIDIManager {
    /// Adds a new managed virtual input to the ``MIDIManager/managedInputs`` dictionary of the ``MIDIManager`` and creates the MIDI port in the system.
    ///
    /// The lifecycle of the MIDI port exists for as long as the ``MIDIManager`` instance exists, or until ``MIDIManager/remove(_:_:)`` is called.
    ///
    /// > A note on `uniqueID`:
    /// >
    ...
    /// >
    /// > Do not generate the ID number yourself - it is always system-generated and then we should store and persist it. ``MIDIIdentifierPersistence`` offers mechanisms to simplify this.
    ///
    /// - Parameters:
