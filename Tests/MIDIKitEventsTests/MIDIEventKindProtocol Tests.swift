//
//  MIDIEventKindProtocol Tests.swift
...
class MIDIEventKindProtocolTests: XCTestCase {
	
	func testEquatable() {
		
		// ensure MIDIEventKindProtocol and AnyMIDIEventKind are equatable
		
		// both 1's should be equal
		let rawKind1 = try! MIDI.Event(rawBytes: [0xFF]).kind
		let anyKind1 = AnyMIDIEventKind(try! MIDI.Event(rawBytes: [0xFF]).kind)
		
		// both 2's should be equal
		let rawKind2 = MIDI.Event(.activeSense).kind
		let anyKind2 = AnyMIDIEventKind(MIDI.Event(.activeSense).kind)
