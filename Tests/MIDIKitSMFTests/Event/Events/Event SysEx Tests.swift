...
final class Event_SysEx_Tests: XCTestCase {
    ...
    // MARK: - MIDIEvent.sysEx(midi1SMFRawBytes:)
    
    func testMIDIEventSysEx_midi1SMFRawBytes_Empty() {
        XCTAssertThrowsError(
            try MIDIEvent.sysEx(midi1SMFRawBytes: [])
        )
    }
    ...
    func testMIDIEventSysEx_midi1SMFRawBytes_SysEx_EmptyData() throws {
        let sysEx = try MIDIEvent.sysEx(midi1SMFRawBytes: [
            0xF0,  // start byte
    ...
    // MARK: - MIDIFileEvent.UniversalSysEx
    ...
    // TODO: SysEx 0xF7 escape message handling
}
