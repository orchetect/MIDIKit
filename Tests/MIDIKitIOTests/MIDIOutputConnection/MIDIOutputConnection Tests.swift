...
extension MIDIOutputConnection_Tests {
    ...
    func outputConnection_allEndpoints() async throws {
        ...
        try await wait(require: { conn.coreMIDIInputEndpointRefs == [input1Ref] }, timeout: 1.0) // TODO: fix TSAN issue
        ...
        try conn.send(event: .start())
        try await wait(require: { await input1Events == [.start()] }, timeout: 0.5)
        #expect(input1Events == [.start()])
        #expect(input2Events == [])
        input1Events = []
        input2Events = []
    ...
    func outputConnection_allEndpoints_filterOwned() async throws {
        ...
        try await Task.sleep(for: .milliseconds(500)) // some time for connection to be notified of new input
